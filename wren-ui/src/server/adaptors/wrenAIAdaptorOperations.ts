      ...
      sql_pairs: [
        {
          ...
          asset_kind: input.sqlPair.assetKind,
          approved_at: input.sqlPair.approvedAt,
          approved_by: input.sqlPair.approvedBy,
          template_level: input.sqlPair.templateLevel,
          template_mode: input.sqlPair.templateMode,
          source_type: input.sqlPair.sourceType,
          scope_type: input.sqlPair.scopeType,
          parameter_schema: input.sqlPair.parameterSchema,
          business_signature: input.sqlPair.businessSignature,
          effective_from: input.sqlPair.effectiveFrom,
          effective_to: input.sqlPair.effectiveTo,
          template_version: input.sqlPair.templateVersion,
          status: input.sqlPair.status,
        },
      ],
  ...
  try {
    const body = {
      instructions: input.instructions.map((item) => ({
        ...
        knowledge_asset_type: item.knowledgeAssetType,
        business_term_id: item.businessTermId,
        external_dependency_id: item.externalDependencyId,
        aliases: item.aliases,
        related_business_terms: item.relatedBusinessTerms,
        related_external_dependencies: item.relatedExternalDependencies,
        runtime_usage: item.runtimeUsage,
        source_status: item.sourceStatus,
        missing_behavior: item.missingBehavior,
        ask_user_prompt: item.askUserPrompt,
        required_grain: item.requiredGrain,
