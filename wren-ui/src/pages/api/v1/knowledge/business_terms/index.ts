...
const logger = getLogger('API_BUSINESS_TERMS');
logger.level = 'debug';
...
interface CreateBusinessTermRequest {
  termId: string;
  name: string;
  category?: string;
  aliases?: string[];
...
const validateBusinessTermInput = (input: CreateBusinessTermRequest) => {
  if (!input.termId?.trim()) {
    throw new ApiError('Business term id is required', 400);
  }
  if (!input.name?.trim()) {
    throw new ApiError('Business term name is required', 400);
  }
};

const handleGetBusinessTerms = async (
  req: NextApiRequest,
  res: NextApiResponse,
  runtimeScope: any,
  startTime: number,
  ...
  const businessTerms =
    await businessKnowledgeService.listBusinessTerms(runtimeIdentity);

  await respondWithSimple({
    res,
    statusCode: 200,
    responsePayload: businessTerms,
    runtimeScope,
    apiType: ApiType.GET_BUSINESS_TERMS,
    startTime,
    requestPayload: {},
...
const handleCreateBusinessTerm = async (
  req: NextApiRequest,
  res: NextApiResponse,
  runtimeScope: any,
  startTime: number,
  ...
  const input = req.body as CreateBusinessTermRequest;
  validateBusinessTermInput(input);
  const runtimeIdentity =
    toCanonicalPersistedRuntimeIdentityFromScope(runtimeScope);
  const actor = buildAuthorizationActorFromRuntimeScope(runtimeScope);
  ...
  const businessTerm = await businessKnowledgeService.createBusinessTerm(
    runtimeIdentity,
    input,
  );
  ...
  await recordAuditEvent({
    ...
    result: 'succeeded',
    context: auditContext,
    payloadJson: { operation: 'business_term.create' },
    afterJson: businessTerm as any,
  });

  await respondWithSimple({
    res,
    statusCode: 201,
    responsePayload: businessTerm,
    runtimeScope,
    apiType: ApiType.CREATE_BUSINESS_TERM,
    startTime,
...
export default async function handler(
  ...
  try {
    ...
    if (req.method === 'GET') {
      await handleGetBusinessTerms(req, res, runtimeScope, startTime);
      return;
    }

    if (req.method === 'POST') {
      await handleCreateBusinessTerm(req, res, runtimeScope, startTime);
      return;
    }
    ...
    await handleApiError({
      ...
      apiType:
        req.method === 'GET'
          ? ApiType.GET_BUSINESS_TERMS
          : ApiType.CREATE_BUSINESS_TERM,
