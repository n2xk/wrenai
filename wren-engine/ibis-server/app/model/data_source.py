...
class DataSourceExtension(Enum):
    ...
    @staticmethod
    def _resolve_optional_value(value):
        if value is None:
            return None
        return (
    ...
    def get_canner_connection(info: CannerConnectionInfo) -> BaseBackend:
        return ibis.postgres.connect(
            ...
            port=int(DataSourceExtension._resolve_value(info.port)),
    ...
    def get_clickhouse_connection(info: ClickHouseConnectionInfo) -> BaseBackend:
        return ibis.clickhouse.connect(
            ...
            user=info.user.get_secret_value(),
            password=DataSourceExtension._resolve_optional_value(info.password),
        )

    @classmethod
    def get_mssql_connection(cls, info: MSSqlConnectionInfo) -> BaseBackend:
        extra_kwargs = getattr(info, "kwargs", None)
        return ibis.mssql.connect(
            host=info.host.get_secret_value(),
            port=str(cls._resolve_value(info.port)),
            database=info.database.get_secret_value(),
            user=info.user.get_secret_value(),
            password=info.password.get_secret_value(),
            driver=info.driver,
            TDS_Version=info.tds_version,
            **extra_kwargs if extra_kwargs else dict(),
        )
    ...
    def get_mysql_connection(cls, info: MySqlConnectionInfo) -> BaseBackend:
        ...
        return ibis.mysql.connect(
            host=info.host.get_secret_value(),
            port=int(cls._resolve_value(info.port)),
            database=info.database.get_secret_value(),
            user=info.user.get_secret_value(),
            password=cls._resolve_optional_value(info.password),
            **kwargs,
        )
    ...
    def get_trino_connection(info: TrinoConnectionInfo) -> BaseBackend:
        return ibis.trino.connect(
            ...
            user=DataSourceExtension._resolve_optional_value(info.user),
            password=DataSourceExtension._resolve_optional_value(info.password),
        )
