...
business_signature:
  ...
  negative_cues:
    - TOP3
    - 首存成本
  expected_grain: dim_game_type_id
source_tables:
...
```sql
WITH dim_game_type_agg AS (
    SELECT
        b.tenant_plat_id,
        b.channel_id,
        b.game_type_id AS dim_game_type_id,
        COUNT(*) AS bet_times,
...
FROM dim_game_type_agg a
LEFT JOIN dim_game_type gt
       ON gt.id = a.dim_game_type_id
ORDER BY a.valid_bet_amount DESC, a.dim_game_type_id;
```
